...
$Oldcode = sanitizeString(Filter_input(INPUT_POST, 'OldC'));
...
if(!empty($productcode) || !empty($productcategory) || !empty($productdescription) || !empty($productsize) || !empty($productcolour) || !empty($productbinnumber) || !empty($productprice))
{
	...
	*/	
		...
		{ 
			...
			$sql = "UPDATE `items` SET `Item_Code` = '$productcode', `Category` = '$productcategory', `Description` = '$productdescription', `Size` = '$productsize', `Colour` = '$productcolour' , `Bin_Number` = '$productbinnumber' , `Item_Price` = '$productprice' WHERE `items`.`Item_Code` = '$Oldcode'"; //SQL code that writes data into the database.
			...
			if($conn->query($sql))		//when information is added to the databse successfully the messgage below is what the user will see.
			{
                           ...
                           header( "Refresh:0; url=AdminEquipmentRentalSystem.php", true, 0);
